	...
	var stub_data = [
		...
		if(!window.localStorage.currencyData || window.localStorage.currencyData == '') {
		    ...
		    var renderContent = '';
    	  	...
    	  	var n = 0;
    	  	var specialClass = " highlightCurrency"; 
		    while(countryList[n]){	
		        ...
		        renderContent += '<tr>'+
							        ...
							        '<td class="rate-text'+(n < 3 ? specialClass : '')+'" onclick="changeBuyingPrice(\''+countryList[n].countryCode+'\', '+countryList[n].buyingRate+', \''+countryList[n].currencyCode+'\')"><i class="fa fa-inr"></i>' + parseFloat(countryList[n].buyingRate).toFixed(2) +'</td>'+
							        '<td class="rate-text'+(n < 3 ? specialClass : '')+'" onclick="changeSellingPrice(\''+countryList[n].countryCode+'\', '+countryList[n].sellingRate+', \''+countryList[n].currencyCode+'\')"><i class="fa fa-inr"></i>' + parseFloat(countryList[n].sellingRate).toFixed(2) +'</td>'+
							      '</tr>';
		        n++;
		    }

		    renderContent = '<col width="10%"><col width="30%"><col width="20%"><col width="20%"><col width="20%">'+
		    	'<thead> <tr class="header-row"> <th></th> <th class="bold" style="font-weight: bold !important;">Country</th> <th class="text-center bold" style="font-weight: bold !important;">Currency</th> <th class="text-center bold" style="font-weight: bold !important; text-align: right; padding-right: 25px !important;">We Buy</th> <th class="text-center bold" style="font-weight: bold !important; text-align: right; padding-right: 25px !important;">We Sell</th> </tr> </thead> <tbody>' + renderContent + '</tbody>';
